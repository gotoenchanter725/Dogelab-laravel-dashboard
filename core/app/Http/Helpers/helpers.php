...
function uniqueIdExists($number) {
    return Account::where('unique_id', $number)->first();
}
...
function sendEmail($user, $type = null, $shortCodes = [])
{
    ...
    if ($config->name == 'php') {
        sendPhpMail($user->email, $user->username, $email_template->subj, $message, $general);
...
function sendContactEmail($email, $name, $subject, $message)
{
    $general = GeneralSetting::first();
    $config = $general->mail_config;
    $receiver_email = $general->email_from;
    $general->email_from = $email;

    $message = shortCodeReplacer("{{message}}", $message, $general->email_template);
    $message = shortCodeReplacer("{{name}}", $general->sitename, $message);


    if ($config->name == 'php') {
        sendPhpMail($receiver_email, $name, $subject, $message, $general);
    } else if ($config->name == 'smtp') {
        sendSmtpMail($config,$receiver_email, $name, $subject, $message, $general);
    } else if ($config->name == 'sendgrid') {
        sendSendGridMail($config, $receiver_email, $name, $subject, $message, $general);
    } else if ($config->name == 'mailjet') {
        sendMailjetMail($config, $receiver_email, $name, $subject, $message, $general);
    }
    return true;
}
...
function sendPhpMail($receiver_email, $receiver_name, $subject, $message, $gnl)
{
...
function sortMonths($array)
{
    for($i = 0; $i < $array->count(); ++$i) {
        $d_val      = Carbon::parse($array[$i]);

        if(isset($array[$i+1])){

            $d_val_next = Carbon::parse($array[$i+1]);

            if($d_val_next < $d_val){
                $temp = $array[$i];
                $array[$i]   = Carbon::parse($array[$i+1])->format('F-Y');
                $array[$i+1] = Carbon::parse($temp)->format('F-Y');
            }else{
                $array[$i]   = Carbon::parse($array[$i])->format('F-Y');
            }
        }
    }
